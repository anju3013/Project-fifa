...
if position == "Goalkeeper":
    ...
    goalkeeping_diving	= st.number_input("Goalkeeping diving", min_value=1, step=1)
    goalkeeping_handling = st.number_input("Goalkeeping Handling",min_value=1, step=1)
    goalkeeping_kicking =st.number_input("Goalkeeping Kicking",min_value=1,step=1)
    goalkeeping_positioning=st.number_input("Goalkeeping Positioning", min_value=1,step=1)
    goalkeeping_reflexes=st.number_input("Goalkeeping Reflexes",min_value=1,step=1)
    
    
    if st.button("Predict Rating"):
        features = [league_rank, international_reputation, weak_foot, body_type_gk.transform([body_type])[0], movement_reactions, power_shot_power, 
    ...
    cam= st.number_input("cam", min_value=1, step=1)
    st_= st.number_input("st", min_value=1, step=1)
    lw= st.number_input("lw", min_value=1, step=1)
    cf= st.number_input("cf", min_value=1, step=1)

    if st.button("Predict Rating"):
        features = [league_rank,  weak_foot,work_rate_fw.transform([work_rate])[0],
                    attacking_volleys,  movement_reactions, power_long_shots,  mentality_positioning,
                    mentality_composure,  passing, dribbling,cam,st_,lw,cf]
    ...
    cb = st.number_input("cb", min_value=1.0, step=1.0)
    lb = st.number_input("lb", min_value=1.0, step=1.0)
    lwb=st.number_input("lwb", min_value=1.0, step=1.0)
    
    if st.button("Predict Rating"):
        features = [league_rank, weak_foot, work_rate_def.transform([work_rate])[0],  attacking_short_passing,
                    skill_ball_control, movement_reactions,  mentality_composure, defending_sliding_tackle, passing, defending, cb,lb,lwb]
    ...
    lam = st.number_input("lam", min_value=1.0, step=1.0)
    lcm = st.number_input("lcm", min_value=1.0, step=1.0)   
    lm=st.number_input("lm", min_value=1.0, step=1.0)   

    if st.button("Predict Rating"):
        features = [league_rank,  weak_foot, work_rate_mid.transform([work_rate])[0],  preferred_foot_mid.transform([preferred_foot])[0], skill_long_passing, 
                     movement_reactions, power_long_shots, mentality_composure, shooting, lam,lcm,lm]
